customElements.define("fit-text", class extends HTMLElement {
    ...
    connectedCallback(// define variables and functions as parameters to save LET declarations
        ...
        if (!this.hasAttribute("static")) {
            ...
            resizeevent = event(window, "resize") // fit when the window resizes
            loadingdoneevent = event(document.fonts, "loadingdone") // fit when fonts load
            ...
            this.disconnect = () => {
                resizeevent() // remove "resize" listener
                loadingdoneevent() // remove font "loadingdone" listener
                observers.map(x => x.disconnect()) // remove all MutationObservers
            }
        }
    }
    disconnectedCallback() {
        this.disconnect && this.disconnect() // remove listeners and observers
    }
});